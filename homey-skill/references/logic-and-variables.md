...
Before removing a variable, run `homeyctl variables usage <name-or-id>`: it searches every ordinary and Advanced Flow for the ID (card arguments, droptokens, embedded tags) and every HomeyScript for the ID or name. `variables usage --prefix AI. --unused` lists AI-built variables nothing references. `variables delete` refuses while a variable is in use unless `--allow-referenced` is given; use that only when the user has decided to break or repair those flows. Dashboards and other apps are not searched, and no static scan can prove an arbitrary script never constructs the ID dynamically. Confirm the deletion target and its effect with the user. Recreated variables have new IDs.
